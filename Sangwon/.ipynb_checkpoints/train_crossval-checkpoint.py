...
def main(args, data_, save_path):
    ...
    for k, (train_index, test_index) in enumerate(skf.split(X, y)):
        ...
        TRAIN_TRANSFORM = transforms.Compose([
                                              transforms.Resize((args.resize[0], args.resize[1])), transforms.ToTensor(),
...
#         model_.load_weights("./darknet53.conv.74") # darknet만
    ...
    # save EachFoldHighAccEpoch
    EachFoldHighAccEpoch = list(map(str, EachFoldHighAccEpoch))
    makeStr = " ".join(EachFoldHighAccEpoch)
    with open("{}/{}/EachFoldHighAccEpoch.txt".format(save_path, args.name), "w") as f:
        f.write(makeStr)
