...
def main(args, test_dir, weight_path):
    EachFoldHighAccEpoch = args.kfold
    if not os.path.exists("{}/{}".format(weight_path, args.name)):
        raise RuntimeError('실험이 존재하지 않음')
    if os.path.exists("{}/{}/EachFoldHighAccEpoch.txt".format(weight_path, args.name)): # 사용할 에포크가 저장되어 있다면 바꿈
        with open("{}/{}/EachFoldHighAccEpoch.txt".format(weight_path, args.name), "r") as f:
            line = f.readline()
            EachFoldHighAccEpoch = line.split(" ")
            EachFoldHighAccEpoch = list(map(int, EachFoldHighAccEpoch))
    ############################################# 데이터 생성 ############################################################
    ...
    for k, epoch in enumerate(EachFoldHighAccEpoch):
    ############################################# 모델 생성 ############################################################
...
#                 pred = pred.argmax(dim=-1) # 확률로 저장할 거임
        ...
        if oof_pred is None:
            oof_pred = fold_pred / len(EachFoldHighAccEpoch)
        else:
            oof_pred += fold_pred / len(EachFoldHighAccEpoch)
